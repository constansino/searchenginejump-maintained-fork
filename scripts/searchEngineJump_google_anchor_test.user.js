...
// @contributor    iqxin
// @description    接手维护版：优化Google结果页挂载位置与稳定性，保留旧版数据兼容
// @version        5.26.8-maintained.1
// @created        2011-07-02
// @lastUpdated    2026-02-16

// @namespace      https://greasyfork.org/zh-CN/scripts/27752-searchenginejump
// @homepage       https://github.com/constansino/searchenginejump-maintained-fork
...
// @downloadURL https://raw.githubusercontent.com/constansino/searchenginejump-maintained-fork/main/scripts/searchEngineJump_google_anchor_test.user.js
// @updateURL https://raw.githubusercontent.com/constansino/searchenginejump-maintained-fork/main/scripts/searchEngineJump_google_anchor_test.user.js
// ==/UserScript==
...
(function () {
    ...
    function iqxinstart(){
        function getGoogleSearchInsertTarget() {
        ...
        var rules = [
            ...
            {name: "google网页搜索",// 你要加载的网站的名字(方便自己查找)
                ...
                insertIntoDoc: {
                    target: function () {
                        return getGoogleSearchInsertTarget();
                    },
                    // 若 keyword 使用函数获取
                    // keyword: function () {
        ...
        function addSEJ(){
            ...
            //固定搜索栏
            if(getSettingData.fixedTop && matchedRule && matchedRule.fixedTop !== false){
                // 判断是否需要只在向上滚动时显示
    ...
    var delayList = [
        /^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}\/search\?/i,
    ...
    if (/^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}\//i.test(url)) {
        var googleLayoutObserverTimer = null;
        new (window.MutationObserver || window.WebKitMutationObserver)(function () {
            if (googleLayoutObserverTimer) clearTimeout(googleLayoutObserverTimer);
            googleLayoutObserverTimer = setTimeout(function () {
                var sejContainer = document.querySelector('#sej-container');
                var anchor = document.querySelector('div.Fgyi2e.rZj61')
                    || document.querySelector('div.Fgyi2e')
                    || document.querySelector('div.GG4mbd');
                if (!anchor) return;
                if (!sejContainer) {
                ...
                if (sejContainer.parentElement !== anchor) {
                    anchor.appendChild(sejContainer);
                }
            }, 60);
